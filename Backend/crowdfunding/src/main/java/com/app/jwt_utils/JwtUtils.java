...
public class JwtUtils {
	...
	//this will be invoked by REST controller(Authentication Controller) upon successful authentication
	public String generateJwtToken(Authentication authentication) {
		...
		return Jwts.builder()//Jwts=factory class used to create jwt tokens
				...
				.setExpiration(new Date((new Date()).getTime()+jwtExpirationMs))//this sets the JWT claims expiration value
				.signWith(SignatureAlgorithm.HS512, jwtSecret)// Signs the constructed JWT using the specified 
				                                              // algorithm with the specified key producing a JWS(Json web signature = signed JWT)
	...
	public boolean validateJwtToken(String authToken) {
		...
		.parseClaimsJws(authToken);// Parses the signed JWT returns the resulting Jws<Claims> instance
		                           //// Parses the signed JWT returns the resulting Jws<Claims> instance
		return true;
		} catch (Exception e) {
